# API Testing with AI Tools - Users API
...
This project demonstrates comprehensive API testing using AI tools and data-driven testing methodologies. The focus is on testing the Users API with various parameters including pagination, user ID, and search functionality.
...
### Core Testing Files:
1. **`collection.json`** - Postman Collection for Users API
2. **`environment.json`** - Postman Environment variables
3. **`user-accounts.csv`** - Data-driven test data (pagination/search parameters)
4. **`run-api-tests.sh`** - Shell script for Newman execution
5. **`run-api-tests.ps1`** - PowerShell script for Windows users

### GitHub Actions:
6. **`api-test.yml`** - Automated testing workflow
7. **`.env.ci`** - CI/CD environment variables

### Test Reports & Documentation:
8. **`22127321_API_Test_Cases_Users.csv`** - 10 comprehensive test cases
9. **`22127321_API_Bug_Report_Users.csv`** - 10 detailed bug reports
10. **`22127321_API_AI_Report_Users.md`** - Individual report (Markdown)
11. **`convert_report_to_pdf.sh`** - Script to convert report to PDF
...
### GitHub Actions:
- **Automated Testing:** Runs on code changes
- **Artifact Management:** Stores test reports
...
4. **BUG004:** Missing Error Handling for Invalid User ID
...
- ✅ **Data-Driven Implementation:** CSV-based testing with 10 iterations
