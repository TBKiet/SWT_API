...
if [ ! -f "collection.json" ]; then
    echo "❌ Collection file not found: collection.json"
    exit 1
fi

if [ ! -f "environment.json" ]; then
    echo "❌ Environment file not found: environment.json"
    exit 1
fi

if [ ! -f "user-accounts.csv" ]; then
    echo "❌ CSV data file not found: user-accounts.csv"
    exit 1
fi
...
newman run collection.json \
    ...
    --iteration-data user-accounts.csv \
